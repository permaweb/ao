...
rm -f "${OUTPUT_DIR}/hyperbeam-*.zip"

deno compile --allow-read --allow-write --allow-run --output "${OUTPUT_DIR}/hb" --target x86_64-unknown-linux-gnu src/mod.js
zip -j "${OUTPUT_DIR}/hyperbeam-x86_64-unknown-linux-gnu.zip" "${OUTPUT_DIR}/hb"
rm "${OUTPUT_DIR}/hb"

deno compile --allow-read --allow-write --allow-run --output "${OUTPUT_DIR}/hb" --target aarch64-apple-darwin src/mod.js
zip -j "${OUTPUT_DIR}/hyperbeam-aarch64-apple-darwin.zip" "${OUTPUT_DIR}/hb"
rm "${OUTPUT_DIR}/hb"

deno compile --allow-read --allow-write --allow-run --output "${OUTPUT_DIR}/hb".exe --target x86_64-pc-windows-msvc src/mod.js
