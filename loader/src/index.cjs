...
globalThis.Extensions = {
  ...
  SHA256: (data) => {
    const d = Arweave.utils.b64UrlToBuffer(data)
    const result = crypto
      .createHash("sha256")
      .update(d)
      .digest()
    
    return Arweave.utils.bufferTob64Url(result)
      
  },
  SigVerify: (owner, data, signature) => {
    const publicKey = {
      kty: "RSA",
      e: "AQAB",
      n: owner,
    ...
    const pem = Arweave.crypto.jwkToPem(publicKey);
    return crypto.createVerify("sha256").update(Arweave.utils.b64UrlToBuffer(data)).verify(
      {
        key: pem,
 ...
 */
module.exports = async function (binary, limit, maxMemory) {
  const instance = await Module(binary, limit, maxMemory);
