...
var Module = (() => {
  ...
  return (
    function(moduleArg = {}) {
      ...
      var /** @type {!Int8Array} */ HEAP8, /** @type {!Uint8Array} */ HEAPU8, /** @type {!Int16Array} */ HEAP16, /** @type {!Uint16Array} */ HEAPU16, /** @type {!Int32Array} */ HEAP32, /** @type {!Uint32Array} */ HEAPU32, /** @type {!Float32Array} */ HEAPF32,
...
/** not-t@type {!BigUint64Array} */
      ...
      var WebGPU = {
        errorCallback: (callback, type, message, userdata) => {
          ...
          ((a1, a2, a3) => {
            throw 'Internal Error! Attempted to invoke wasm function pointer with signature "vijj", but no such functions have gotten exported!';
          })(type, messagePtr, userdata);
          stackRestore(sp);
        },
        initManagers: () => {
        ...
        makeImageCopyTexture: ptr => {
          assert(ptr);
          assert(Number(HEAPU64[((ptr) / 8)]) === 0);
          return {
            "texture": WebGPU.mgrTexture.get(Number(HEAPU64[(((ptr) + (8)) / 8)])),
            "mipLevel": HEAPU32[(((ptr) + (16)) / 4)],
            "origin": WebGPU.makeOrigin3D(ptr + 20),
            "aspect": WebGPU.TextureAspect[HEAPU32[(((ptr) + (32)) / 4)]]
          };
        },
        makeTextureDataLayout: ptr => {
          assert(ptr);
          assert(Number(HEAPU64[((ptr) / 8)]) === 0);
          var bytesPerRow = HEAPU32[(((ptr) + (16)) / 4)];
          var rowsPerImage = HEAPU32[(((ptr) + (20)) / 4)];
          return {
            "offset": HEAPU32[((((ptr + 4)) + (8)) / 4)] * 4294967296 + HEAPU32[(((ptr) + (8)) / 4)],
        ...
        makeImageCopyBuffer: ptr => {
          ...
          bufferCopyView["buffer"] = WebGPU.mgrBuffer.get(Number(HEAPU64[(((ptr) + (32)) / 8)]));
        ...
        makePipelineConstants: (constantCount, constantsPtr) => {
          ...
          for (var i = 0; i < constantCount; ++i) {
            ...
            var key = UTF8ToString(Number(HEAPU64[(((entryPtr) + (8)) / 8)]));
            constants[key] = HEAPF64[(((entryPtr) + (16)) / 8)];
          }
          return constants;
        },
        ...
        makeProgrammableStageDescriptor: ptr => {
          ...
          assert(Number(HEAPU64[((ptr) / 8)]) === 0);
          var desc = {
            "module": WebGPU.mgrShaderModule.get(Number(HEAPU64[(((ptr) + (8)) / 8)])),
            "constants": WebGPU.makePipelineConstants(HEAPU32[(((ptr) + (24)) / 4)], Number(HEAPU64[(((ptr) + (32)) / 8)]))
          };
          var entryPointPtr = Number(HEAPU64[(((ptr) + (16)) / 8)]);
      ...
      var _emscripten_webgpu_get_device = function() {
        var ret = (() => {
      ...
      var _wgpuBufferGetConstMappedRange = function(bufferId, offset, size) {
        bufferId = bigintToI53Checked(bufferId);
        offset = bigintToI53Checked(offset);
      ...
      function _wgpuCommandEncoderCopyTextureToBuffer(encoderId, srcPtr, dstPtr, copySizePtr) {
        ...
        copySizePtr = bigintToI53Checked(copySizePtr);
        var commandEncoder = WebGPU.mgrCommandEncoder.get(encoderId);
      ...
      var _wgpuDeviceCreateBindGroupLayout = function(deviceId, descriptor) {
        ...
        var ret = (() => {
          ...
          function makeBufferEntry(entryPtr) {
            assert(entryPtr);
            var typeInt = HEAPU32[(((entryPtr) + (8)) / 4)];
            if (!typeInt) return undefined;
            return {
              "type": WebGPU.BufferBindingType[typeInt],
              "hasDynamicOffset": !!(HEAPU32[(((entryPtr) + (12)) / 4)]),
              "minBindingSize": HEAPU32[((((entryPtr + 4)) + (16)) / 4)] * 4294967296 + HEAPU32[(((entryPtr) + (16)) / 4)]
            };
          }
          ...
          function makeTextureEntry(entryPtr) {
            assert(entryPtr);
            var sampleTypeInt = HEAPU32[(((entryPtr) + (8)) / 4)];
            if (!sampleTypeInt) return undefined;
            return {
              "sampleType": WebGPU.TextureSampleType[sampleTypeInt],
              "viewDimension": WebGPU.TextureViewDimension[HEAPU32[(((entryPtr) + (12)) / 4)]],
              "multisampled": !!(HEAPU32[(((entryPtr) + (16)) / 4)])
            };
          }

          function makeStorageTextureEntry(entryPtr) {
            assert(entryPtr);
            var accessInt = HEAPU32[(((entryPtr) + (8)) / 4)];
            if (!accessInt) return undefined;
            return {
              "access": WebGPU.StorageTextureAccess[accessInt],
              "format": WebGPU.TextureFormat[HEAPU32[(((entryPtr) + (12)) / 4)]],
              "viewDimension": WebGPU.TextureViewDimension[HEAPU32[(((entryPtr) + (16)) / 4)]]
            };
          }
          ...
          function makeEntry(entryPtr) {
            assert(entryPtr);
            return {
              "binding": HEAPU32[(((entryPtr) + (8)) / 4)],
              "visibility": HEAPU32[(((entryPtr) + (12)) / 4)],
              "buffer": makeBufferEntry(entryPtr + 16),
              "sampler": makeSamplerEntry(entryPtr + 40),
              "texture": makeTextureEntry(entryPtr + 56),
              "storageTexture": makeStorageTextureEntry(entryPtr + 80)
            };
          }
          ...
          function makeEntries(count, entriesPtrs) {
            var entries = [];
            for (var i = 0; i < count; ++i) {
              entries.push(makeEntry(entriesPtrs + 104 * i));
            }
            return entries;
          }
          var desc = {
            "entries": makeEntries(HEAPU32[(((descriptor) + (16)) / 4)], Number(HEAPU64[(((descriptor) + (24)) / 8)]))
          };
          var labelPtr = Number(HEAPU64[(((descriptor) + (8)) / 8)]);
          if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
      ...
      var _wgpuDeviceCreateBuffer = function(deviceId, descriptor) {
        deviceId = bigintToI53Checked(deviceId);
        descriptor = bigintToI53Checked(descriptor);
        var ret = (() => {
          assert(descriptor);
          assert(Number(HEAPU64[((descriptor) / 8)]) === 0);
          var mappedAtCreation = !!(HEAPU32[(((descriptor) + (32)) / 4)]);
          var desc = {
            "label": undefined,
            "usage": HEAPU32[(((descriptor) + (16)) / 4)],
            "size": HEAPU32[((((descriptor + 4)) + (24)) / 4)] * 4294967296 + HEAPU32[(((descriptor) + (24)) / 4)],
      ...
      var _wgpuDeviceCreateCommandEncoder = function(deviceId, descriptor) {
        deviceId = bigintToI53Checked(deviceId);
        descriptor = bigintToI53Checked(descriptor);
        var ret = (() => {
          var desc;
      ...
      var _wgpuDeviceCreatePipelineLayout = function(deviceId, descriptor) {
        deviceId = bigintToI53Checked(deviceId);
        descriptor = bigintToI53Checked(descriptor);
        var ret = (() => {
          assert(descriptor);
          assert(Number(HEAPU64[((descriptor) / 8)]) === 0);
          var bglCount = HEAPU32[(((descriptor) + (16)) / 4)];
          var bglPtr = Number(HEAPU64[(((descriptor) + (24)) / 8)]);
          var bgls = [];
          for (var i = 0; i < bglCount; ++i) {
            bgls.push(WebGPU.mgrBindGroupLayout.get(Number(HEAPU64[(((bglPtr) + (8 * i)) / 8)])));
          }
          var desc = {
            "label": undefined,
      ...
      var _wgpuDeviceCreateTexture = function(deviceId, descriptor) {
        deviceId = bigintToI53Checked(deviceId);
        descriptor = bigintToI53Checked(descriptor);
        var ret = (() => {
          assert(descriptor);
          ...
          var desc = {
            "label": undefined,
            "size": WebGPU.makeExtent3D(descriptor + 24),
            "mipLevelCount": HEAPU32[(((descriptor) + (40)) / 4)],
            "sampleCount": HEAPU32[(((descriptor) + (44)) / 4)],
            "dimension": WebGPU.TextureDimension[HEAPU32[(((descriptor) + (20)) / 4)]],
            "format": WebGPU.TextureFormat[HEAPU32[(((descriptor) + (36)) / 4)]],
            "usage": HEAPU32[(((descriptor) + (16)) / 4)]
          };
          var labelPtr = Number(HEAPU64[(((descriptor) + (8)) / 8)]);
          if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
          var viewFormatCount = HEAPU32[(((descriptor) + (48)) / 4)];
          if (viewFormatCount) {
            var viewFormatsPtr = Number(HEAPU64[(((descriptor) + (56)) / 8)]);
            desc["viewFormats"] = Array.from(HEAP32.subarray((((viewFormatsPtr) / 4)), ((viewFormatsPtr + viewFormatCount * 4) / 4)), function(format) {
      ...
      var _wgpuDeviceGetQueue = function(deviceId) {
        deviceId = bigintToI53Checked(deviceId);
        var ret = (() => {
      ...
      var _wgpuTextureCreateView = function(textureId, descriptor) {
        ...
        var ret = (() => {
          ...
          if (descriptor) {
            assert(descriptor);
            assert(Number(HEAPU64[((descriptor) / 8)]) === 0);
            var mipLevelCount = HEAPU32[(((descriptor) + (28)) / 4)];
            var arrayLayerCount = HEAPU32[(((descriptor) + (36)) / 4)];
            desc = {
              "format": WebGPU.TextureFormat[HEAPU32[(((descriptor) + (16)) / 4)]],
              "dimension": WebGPU.TextureViewDimension[HEAPU32[(((descriptor) + (20)) / 4)]],
              "baseMipLevel": HEAPU32[(((descriptor) + (24)) / 4)],
              "mipLevelCount": mipLevelCount === 4294967295 ? undefined : mipLevelCount,
              "baseArrayLayer": HEAPU32[(((descriptor) + (32)) / 4)],
              "arrayLayerCount": arrayLayerCount === 4294967295 ? undefined : arrayLayerCount,
              "aspect": WebGPU.TextureAspect[HEAPU32[(((descriptor) + (40)) / 4)]]
            };
            var labelPtr = Number(HEAPU64[(((descriptor) + (8)) / 8)]);
            if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
          }
      ...
      var wasmImports = {
        ...
        /** @export */
        __asyncjs__weavedrive_close: __asyncjs__weavedrive_close,
        /** @export */
        __asyncjs__weavedrive_open: __asyncjs__weavedrive_open,
        /** @export */
        __asyncjs__weavedrive_read: __asyncjs__weavedrive_read,
        /** @export */
        ...
        /** @export */
        metering_gasUsed: metering_gasUsed,
        /** @export */
        strftime: _strftime,
        /** @export */
