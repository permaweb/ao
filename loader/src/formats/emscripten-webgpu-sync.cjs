...
var Module = (() => {
  ...
  return (
    function(moduleArg = {}) {
      ...
      var _emscripten_get_now = () => deterministicNow();
      ...
      var WebGPU = {
        ...
        errorCallback: (callback, type, message, userdata) => {
          ...
          ((a1, a2, a3) => {
            throw 'Internal Error! Attempted to invoke wasm function pointer with signature "viii", but no such functions have gotten exported!';
        ...
        fillLimitStruct: (limits, supportedLimitsOutPtr) => {
          ...
          function setLimitValueU64(name, limitOffset) {
            ...
            (tempI64 = [limitValue >>> 0, (tempDouble = limitValue, (+(Math.abs(tempDouble))) >= 1 ? (tempDouble > 0 ? (+(Math.floor((tempDouble) / 4294967296))) >>> 0 : (~~((+(Math.ceil((tempDouble - +(((~~(tempDouble))) >>> 0)) / 4294967296))))) >>> 0) : 0)],
              HEAP32[(((limitsOutPtr) + (limitOffset)) >> 2)] = tempI64[0], HEAP32[(((limitsOutPtr) + ((limitOffset) + (4))) >> 2)] = tempI64[1]);
          }
