...
const binaryPath = './test/emscripten4/process.wasm'
const wasmBinary = fs.readFileSync(binaryPath)

const options = { format: 'wasm32-unknown-emscripten-metering' } // Same binary as wasm32-unknown-emscripten4 but with metering
...
describe('loader', async () => {
  ...
  it('should refill the gas on every invocation', async () => {
     ...
     * but they should be within ~10.4B of each other, effectively meaning the gas is not
     * "stacking" and being refilled every time
     */
    const gasDiff = Math.abs(result.GasUsed - result2.GasUsed)
    assert.ok(gasDiff < 10400000000)
  })
