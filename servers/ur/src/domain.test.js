...
describe('domain', () => {
  describe('determineHostWith', () => {
    describe('compute or cached', () => {
      ...
      test('should roughly evenly distribute hashes across the length', async () => {
        const HOSTS = Array(15).fill(0).map((_, i) => i)
        const determineHost = determineHostWith({ hosts: HOSTS })
        ...
        const mappings = await Promise.all(hashes.map(hash => determineHost({ processId: hash })))

        const res = Object.values(groupBy(identity, mappings)).map(a => a.length)
