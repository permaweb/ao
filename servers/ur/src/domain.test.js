...
describe('domain', () => {
  describe('determineHostWith', () => {
    test('should deterministically return a valid host', async () => {
      ...
      assert(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }))
      assert.equal(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }), await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }))
    })

    test('should shift the determined host according to failoverAttempt', async () => {
      const determineHost = determineHostWith({ hosts: HOSTS, cache })

      assert.notEqual(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }), await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 1 }))
    })

    test('should return undefined if all hosts have been attempted', async () => {
      const determineHost = determineHostWith({ hosts: HOSTS, cache })
      assert.equal(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: HOSTS.length }), undefined)
    })
    ...
    test('should serve from the cache, if found', async () => {
      ...
      assert.equal(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: HOSTS.length }), HOSTS[HOSTS.length & 10])
    })

    test('should redirect to the subrouterUrl', async () => {
      const fetchMock = async (url) => {
        assert.equal(url, 'surUrl1/processes/0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0')
      ...
      assert(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }))
      assert.equal(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }), 'subrouterUrl1')
    })
    ...
    test('should not redirect to the subrouterUrl', async () => {
      ...
      assert(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }))
      assert.equal(await determineHost({ processId: '0h1DJuO1SBNOR54ZN-rwt0Ztenpv7us1XKDxhLMzYq0', failoverAttempt: 0 }), 'http://fizz.buzz')
    })
  })
  ...
  describe('computeHashSumFromProcessId', () => {
    ...
    test('should roughly evenly distribute hashes across the length', () => {
      ...
      const res = Object.values(
        groupBy(
          identity,
          hashes.map(h => computeHashSumFromProcessId({ processId: h, length: 24 }))
        )
      ).map(a => a.length)
