    ...
    if (ctx.schedLocation) {
      ...
      return writeDataItem({ suUrl: ctx.schedLocation.url, data: ctx.tx.data.toString('base64') })
        ...
        .bimap(
          ...
          logger.tap({ log: 'Added "schedulerTx" to ctx', options: { messageId: ctx.messageId, processId: ctx.processId } })
        )
    } else {
      ...
      return writeDataItemArweave(ctx.tx.data)
        ...
        .bimap(
          ...
          logger.tap({ log: 'Added "arweaveTx" to ctx', options: { messageId: ctx.messageId, processId: ctx.processId } })
        )
    }
  }
}
