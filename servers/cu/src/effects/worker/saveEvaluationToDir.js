...
export function saveEvaluationToDirWith ({ EVALUATION_RESULT_DIR, EVALUATION_RESULT_BUCKET }) {
  return async ({ messageId, processId, output }) => {
    if (!EVALUATION_RESULT_DIR || !EVALUATION_RESULT_BUCKET) {
      console.log('SAVE: EVALUATION_RESULT_DIR or EVALUATION_RESULT_BUCKET is not set')
      return 'not set'
    }
    ...
    if (!fs.existsSync(dir)) {
      console.log('SAVE: SAVING EVALUATION TO DIR', { EVALUATION_RESULT_DIR, EVALUATION_RESULT_BUCKET, messageId })
      try {
        fs.writeFileSync(dir, JSON.stringify(outputWithoutMemory))
      } catch (e) {
        console.error('SAVE: Error writing file', e)
      }
    } else {
      console.log(`SAVE: File ${dir} already exists, skipping`)
    }
    return output
  }
}
