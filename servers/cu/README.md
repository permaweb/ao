...
- `EAGER_CHECKPOINT_EVAL_TIME_THRESHOLD`: If a process has this amount of eval
  ...
  evaluation stream.
- `MID_EVALUATION_CHECKPOINTING`: When set to `'true'`, allows checkpoints to be created during message evaluation streams rather than only at the end. This creates intermediate checkpoints whenever gas usage or evaluation time thresholds are reached, improving recovery after interruptions by avoiding the need to re-process messages from the beginning (defaults to `false`).
