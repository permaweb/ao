...
use crate::domain::core::dal::{DataStore, Process, StoreErrorType, Gateway};
...
pub async fn sync_local_drives() -> io::Result<()> {
    ...
    for process_id in process_keys {
       ...
       */
       let latest_message_synced = match process_keys_sync.contains(&process_id) {
          true => {
            match write_sync_store.get_latest_message(&process_id).await {
              Ok(m) => m,
              Err(_) => None
            }
          },
          false => {
            ...
            */
            ...
            let process_bundle = read_only_local_store
              .file_db.get(assignment_key.as_bytes())
              .unwrap()
              .unwrap();

            write_sync_store.save_process(&process_to_sync, &process_bundle).unwrap();
       ...
       let mut from = match latest_message_synced {
          Some(message) => {
            Some(message.timestamp().unwrap().to_string())
          },
          None => {
            None
          }
      ...
      while has_next_page {
          ...
          for message in messages_fetch.edges.iter() {
              ...
              let bundle_data_item = read_only_local_store.file_db.get(
                assignment_key.as_bytes()
              ).unwrap().unwrap();
              
              match write_sync_store.get_message(
                &message.node.assignment_id().unwrap()
              ) {
                Err(StoreErrorType::NotFound(_)) => {
                  let deep_hash = msg_deephash(
                    gw, 
                    &message.node, 
                    &bundle_data_item
                  ).await.unwrap();

                  write_sync_store.save_message(
                    &message.node, 
                    &bundle_data_item, 
                    Some(&deep_hash)
                  ).await.unwrap();
