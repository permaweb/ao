...
impl DataItem {
    ...
    fn verify_ethereum(&mut self) -> Result<(), ByteErrorType> {
        ...
        if self.owner.len() != 65 {
            return Err(ByteErrorType::ByteError(
                "Invalid owner format for Ethereum signature".to_string(),
            ));
        }
        ...
        let recovery_id = match v {
            ...
            _ => {
                return Err(ByteErrorType::ByteError(
                    "Invalid recovery ID for Ethereum signature".to_string(),
                ));
            }
        };
        ...
        let recovered_key =
            match VerifyingKey::recover_from_prehash(msg_hash.as_slice(), &signature, rec_id) {
                Ok(key) => key,
                Err(_) => {
                    return Err(ByteErrorType::ByteError(
                        "Failed to recover public key".to_string(),
                    ));
                }
            };
        ...
        if !addresses_match {
            return Err(ByteErrorType::ByteError(
                "Ethereum signature verification failed".to_string(),
            ));
        }
