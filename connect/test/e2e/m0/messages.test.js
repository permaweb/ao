...
test('Get Messages by Range', async () => {
  const { getMessages } = connect({ MODE: 'mainnet', signer: createSigner(WALLET) })

  const PROCESS = '0syT13r0s0tgPmIed95bJnuSqaD29HQNN8D3ElLSrsc'
  const from = String(Date.now() - (10 * 60 * 1000))
  const to = String(Date.now())
  const limit = String(1000)
  const results = await getMessages({ processId: PROCESS, from, to, limit })
