...
test.skip('get: redirect request', async () => {
  const server = http.createServer((req, res) => {
    res.writeHead(422, {
      'Content-Encoding': 'http',
      'Accept-Device': 'process@1.0',
      Location: 'https://10000-permaweb-hbinfra-erkeu448sa9.ws-us117.gitpod.io'
    })
    res.end()
  }).listen(14000)
  ...
  const { get } = connect({ MODE: 'mainnet', URL: 'http://localhost:14000', wallet: WALLET })
  try {
    ...
    assert.equal(e.name, 'RedirectRequested')
    assert.equal(e.device, 'process@1.0')
    assert.equal(e.contentEncoding, 'html')
    assert.equal(e.location, 'https://10000-permaweb-hbinfra-erkeu448sa9.ws-us117.gitpod.io')
