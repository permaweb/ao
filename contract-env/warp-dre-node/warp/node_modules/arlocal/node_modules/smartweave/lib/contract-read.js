...
function readContract(arweave, contractId, height, returnValidity) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const loadPromise = (0, contract_load_1.loadContract)(arweave, contractId).catch((err) => {
            const error = new errors_1.default("CONTRACT_NOT_FOUND" /* SmartWeaveErrorType.CONTRACT_NOT_FOUND */, {
        ...
        (0, utils_1.log)(arweave, `Replaying ${txInfos.length} confirmed interactions`);
        ...
        for (const txInfo of txInfos) {
            ...
            if ((0, utils_1.hasMultipleinteractions)(txInfo)) {
                ...
                if (!inputTag || inputTag.name !== 'Input') {
                    (0, utils_1.log)(arweave, `Skipping tx with missing or invalid Input tag - ${currentTx.id}`);
                    continue;
                }
            ...
            const result = yield (0, contract_step_1.execute)(handler, interaction, state);
            if (result.type === 'exception') {
                (0, utils_1.log)(arweave, `${result.result}`);
                (0, utils_1.log)(arweave, `Executing of interaction: ${currentTx.id} threw exception.`);
            }
            if (result.type === 'error') {
                (0, utils_1.log)(arweave, `${result.result}`);
                (0, utils_1.log)(arweave, `Executing of interaction: ${currentTx.id} returned error.`);
            }
            validity[currentTx.id] = result.type === 'ok';
            state = result.state;
            const settings = (0, utils_1.evalSettings)(state);
            ...
            if (evolve && /[a-z0-9_-]{43}/i.test(evolve) && canEvolve && contractSrcTXID !== evolve) {
                try {
                    contractInfo = yield (0, contract_load_1.loadContract)(arweave, contractId, evolve);
...
function addSortKey(arweave, txInfo) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const hashed = (0, utils_1.arrayToHex)(yield arweave_1.default.crypto.hash(concatted));
...
function fetchTransactions(arweave, contractId, height) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const txInfos = transactions.edges.filter((tx) => { var _a, _b; return !tx.node.bundledIn || !((_a = tx.node.bundledIn) === null || _a === void 0 ? void 0 : _a.id) || !tx.node.parent || !((_b = tx.node.parent) === null || _b === void 0 ? void 0 : _b.id); });
        while (transactions.pageInfo.hasNextPage) {
            ...
            txInfos.push(...transactions.edges.filter((tx) => { var _a, _b; return !tx.node.bundledIn || !((_a = tx.node.bundledIn) === null || _a === void 0 ? void 0 : _a.id) || !tx.node.parent || !((_b = tx.node.parent) === null || _b === void 0 ? void 0 : _b.id); }));
        }
        return txInfos;
    });
}
