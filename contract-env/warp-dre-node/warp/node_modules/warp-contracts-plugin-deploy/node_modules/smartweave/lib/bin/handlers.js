...
function readCommandHandler(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const jsonInput = (0, utils_2.getJsonInput)(input);
    ...
    `);
            argv.prettify
                ? console.log((0, json_beautify_1.default)(result, null, 2, 100))
                : console.log(result, `
...
function writeCommandHandler(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        status.start();
        try {
            wallet = JSON.parse((0, fs_1.readFileSync)(argv.keyFile).toString());
            status.stop();
        }
        catch (err) {
            status.stop();
    ...
    `);
      ...
      `);
                const resp = yield (0, inquirer_1.askForContractInteractionConfirmation)(confirmRandomWord, expectedContractInteractionFee);
...
function createCommandHandler(argv) {
    ...
    `);
        ...
        status.start();
        if (!(0, utils_2.isExpectedType)(initStateFile, 'json')) {
            status.stop();
            loglevel_1.default.error(`
    ...
    `);
        ...
        if (contractSource.includes('.')) {
            ...
            status.start();
            if (!(0, fs_1.existsSync)(contractSource)) {
                status.stop();
                loglevel_1.default.error(`
      ...
      `);
            ...
            if (!(0, utils_2.isExpectedType)(contractSource, 'js')) {
                status.stop();
                loglevel_1.default.error(`
      ...
      `);
            ...
            status.start();
            const tx = yield Sdk.simulateCreateContractFromSource(arweave, wallet, (0, fs_1.readFileSync)(initStateFile).toString(), (0, fs_1.readFileSync)(contractSource).toString());
            const userAddress = yield arweave.wallets.jwkToAddress(wallet);
            const userBalance = arweave.ar.winstonToAr(yield arweave.wallets.getBalance(userAddress));
    ...
    `);
            const resp = yield (0, inquirer_1.askForContractCreationConfirmation)(confirmRandomWord, expectedContractCreationFee);
      ...
      `);
            ...
            status.start();
            try {
                const contractId = yield Sdk.createContract(arweave, wallet, (0, fs_1.readFileSync)(contractSource).toString(), (0, fs_1.readFileSync)(initStateFile).toString());
      ...
      `);
            ...
            try {
                ...
                const appTag = (0, utils_1.getTag)(sourceTx, 'App-Name');
                // assert(
      ...
      `);
            ...
            try {
                ...
                status = new Spinner(`Computing the fee needed for creating your contract, please wait...`);
                status.start();
                const tx = yield Sdk.simulateCreateContractFromTx(arweave, wallet, sourceTx.id, (0, fs_1.readFileSync)(initStateFile).toString());
                status.stop();
                const userAddress = yield arweave.wallets.jwkToAddress(wallet);
                const userBalance = arweave.ar.winstonToAr(yield arweave.wallets.getBalance(userAddress));
                const expectedContractCreationFee = yield arweave.ar.winstonToAr(tx.reward);
      ...
      `);
        ...
        `);
                ...
                status = new Spinner(`Amazing! Let me deploy your contract, please wait...`);
                status.start();
                const contractId = yield Sdk.createContractFromTx(arweave, wallet, sourceTx.id, (0, fs_1.readFileSync)(initStateFile).toString());
                status.stop();
                console.log(`     🥳 ${chalk_1.default.green(`Your contract with ID ${chalk_1.default.bgBlack(chalk_1.default.white(contractId))} was successfully deployed on the arweave!`)} 🥳
