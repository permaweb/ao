 ...
 */
function interactWrite(arweave, wallet, contractId, input, tags = [], target = '', winstonQty = '', reward) {
    return __awaiter(this, void 0, void 0, function* () {
        const interactionTx = yield createTx(arweave, wallet, contractId, input, tags, target, winstonQty, reward);
...
function interactWriteDryRun(arweave, wallet, contractId, input, tags = [], target = '', winstonQty = '', myState, fromParam, contractInfoParam) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let { handler, swGlobal, contractSrcTXID } = contractInfoParam || (yield (0, contract_load_1.loadContract)(arweave, contractId));
        const latestState = myState || (yield (0, contract_read_1.readContract)(arweave, contractId));
        const from = fromParam || (yield arweave.wallets.getAddress(wallet));
        const settings = (0, utils_1.evalSettings)(latestState);
        ...
        if (evolve && /[a-z0-9_-]{43}/i.test(evolve) && canEvolve) {
            if (contractSrcTXID !== evolve) {
                try {
                    const contractInfo = yield (0, contract_load_1.loadContract)(arweave, contractId, evolve);
...
function interactRead(arweave, wallet, contractId, input, tags = [], target = '', winstonQty = '') {
    return __awaiter(this, void 0, void 0, function* () {
        // tslint:disable-next-line: prefer-const
        let { handler, swGlobal, contractSrcTXID } = yield (0, contract_load_1.loadContract)(arweave, contractId);
        const latestState = yield (0, contract_read_1.readContract)(arweave, contractId);
        ...
        const currentBlock = yield arweave.blocks.getCurrent();
        swGlobal._activeTx = createDummyTx(tx, from, ts, currentBlock);
        const result = yield (0, contract_step_1.execute)(handler, interaction, latestState);
