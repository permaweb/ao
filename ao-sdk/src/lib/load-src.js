...
import { pathOr, reduce, assoc, prop } from 'ramda'
...
function getSourceBuffer(srcId) {
  return fromPromise(id => fetch(`${GATEWAY_URL}/${id}`)
    .then(res => res.arrayBuffer()))(srcId)
}

function getSourceId(id) {
  return fromPromise(id => fetch(`${GATEWAY_URL}/graphql`, { 
    method: 'POST', 
    headers: { 'Content-Type' : 'application/json' },
    body: JSON.stringify({query: buildContractQuery(id)})
  })
    .then(res => res.json())
    .then(pathOr(null, ['data', 'transactions', 'edges', '0', 'node', 'tags']))
